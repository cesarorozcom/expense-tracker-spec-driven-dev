...
def validate_photo_file(file):
    ...
    if file.size > 10 * 1024 * 1024:
        raise ValidationError('Photo size must be under 10MB.')
...
class AccountForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['name']
        widgets = {
            'name': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Account name (e.g., Checking)'}),
        }
...
class TransactionForm(forms.ModelForm):
    photo = forms.ImageField(
        ...
        help_text='Upload a receipt photo (JPEG, PNG, GIF, or WebP). Max 10MB.'
    )

    class Meta:
        model = Transaction
        fields = ['account', 'occurred_at', 'amount', 'description', 'photo', 'is_recurring']
        widgets = {
